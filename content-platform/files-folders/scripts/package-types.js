...
function getTypeFilePath(sourcePath) {
  // TypeScript generates types preserving directory structure
  // Try multiple possible locations
  const relativePath = sourcePath.replace('./src/', '');

  const possiblePaths = [
    path.join(TYPES_DIR, `${relativePath}.d.ts`),
    path.join(TYPES_DIR, 'files-folders', sourcePath.replace('./', '') + '.d.ts'),
    path.join(TYPES_DIR, 'files-folders', sourcePath + '.d.ts'),
  ];
...
function getModuleName(exposeName) {
  return `files_tab${exposeName.replace('./', '/')}`;
}
...
function main() {
  console.log('📦 Packaging Module Federation types for files-tab...\n');
